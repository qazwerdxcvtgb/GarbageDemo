...
slot.CanAccept(ItemCard card)                      // 仅接受 FishData 类型
```
...
4. `ShopHangController` 执行：
   - **场景2（槽位有卡）**：先调用 `EjectCardToHand` 将旧卡归还手牌
...
3. `ShopHangController.EjectCardToHand` 执行：
...
```csharp
EquipmentPanel.Instance.OpenPanel()
EquipmentPanel.Instance.ClosePanel()
EquipmentPanel.Instance.TogglePanel()   // 绑定按钮 onClick
```
