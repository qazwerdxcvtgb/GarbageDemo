...
| 等级 | 疯狂值范围 | 纯净鱼售价修正 | 污秽鱼售价修正 | 体力上限基准 |
|------|-----------|---------------|---------------|-------------|
| Level0 | 0 | +2 | -2 | 12 |
| Level1 | 1–3 | +1 | -1 | 15 |
| Level2 | 4–6 | +1 | 0 | 15 |
...
- **售价修正**：仅影响 `FishData` 类型卡牌，修正值叠加到 `ItemData.value` 上（单卡最低 0）。`ShopSellController` 在计算总价和执行售卖时均使用修正后价格。
- **体力上限**：`CharacterState` 订阅 `OnSanityLevelChanged`，通过级间差值调整 `MaxHealth`。升级增加上限，降级减少上限并自动钳制当前体力不超过新上限。
...
GameManager.Instance.GetSanityGoldModifier(FishType) // 获取当前等级下指定鱼类型的售价修正
GameManager.Instance.GetMaxHealthForSanityLevel(SanityLevel) // 获取指定等级的体力上限基准值
```
...
```csharp
characterState.MaxHealth      // 最大体力（受疯狂等级级间差值影响）
...
> **疯狂等级联动体力上限**：`CharacterState` 在 `Start` 中订阅 `GameManager.OnSanityLevelChanged`，
> 等级变化时计算级间差值调整 `MaxHealth`（如 Level0→Level1 = +3，Level1→Level0 = -3）。
> 降级时 `MaxHealth` setter 自动钳制 `currentHealth` 不超过新上限。
> 其他来源的 `MaxHealth` 加成（D6 每日 +3、`Effect_ModifyMaxHealth`）不受影响。
